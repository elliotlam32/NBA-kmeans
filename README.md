...
Issues/Solutions
The K-Means graph is quite difficult to understand, especially since the centroids aren’t labeled, and the graph tends to pick dots that are very similar in color.
Overly Specialzied clusters, picking individuals that excel at their archetype to cluster around creawteas a cluster that almost always excludes average players of that archetype - for example choosing wemby to cluster around almost always produced a cluster of just Wemby and Chet Holmgren

Improvements/Potential Areas to Build
Execute the same project but on data from the 2018-2019 season, to see which players changed groups/archetypes
Also provide qualitative analysis as to how and potentially why? Any pioneers in the field?
